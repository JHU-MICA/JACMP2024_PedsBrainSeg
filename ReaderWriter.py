...
class DicomReaderWriter(object):
    ...
    def __compile__(self):
        ...
        for rt_series_instance_uid in self.rt_dictionary:
            ...
            if series_instance_uid in series_instance_uids:
                ...
                self.series_instances_dictionary[index]['RTs'].update(
                    {rt_series_instance_uid: self.rt_dictionary[rt_series_instance_uid]})
            else:
                while index in self.series_instances_dictionary:
                    index += 1
    ...
    def mask_to_contours(self) -> None:
        ...
        self.struct_index = 0
        new_ROINumber = 1000
        for cnum, Name in enumerate(self.ROI_Names):
            new_ROINumber -= 1
            if not temp_color_list:
                temp_color_list = copy.deepcopy(color_list)
            color_int = cnum  # int(self.ROI_Names.index(Name)) #np.random.randint(len(temp_color_list))
            print('Writing data for n ' + Name)
        ...
        today = date.today()
        self.RS_struct.InstanceCreationDate = today.strftime("%Y%m%d")
        self.RS_struct.StructureSetDate = today.strftime("%Y%m%d")

        now = datetime.now()
        self.RS_struct.InstanceCreationTime = now.strftime("%H%M%S")
        self.RS_struct.StructureSetTime = now.strftime("%H%M%S")
        self.RS_struct.SeriesTime = now.strftime("%H%M%S")

        self.RS_struct.StudyDescription = 'MICA_Peds_AutoSeg'
        self.RS_struct.SeriesDescription = 'MICA_Peds_AutoSeg'
        self.RS_struct.PatientIdentityRemoved = 'None'
        self.RS_struct.StructureSetLabel = 'MICA_Peds_AutoSeg'
        self.RS_struct.StructureSetName = 'MICA_Peds_AutoSeg'

        self.RS_struct.SoftwareVersions = 'None'
